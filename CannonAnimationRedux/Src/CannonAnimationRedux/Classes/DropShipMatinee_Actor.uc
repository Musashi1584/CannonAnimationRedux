	...
	Begin:
		while (true)
		{
			if (LatestPatchedStreamingMaps != `MAPS.NumStreamingMaps() && `MAPS.IsStreamingComplete())
			{
				...
				`log("New number of streaming maps:" @ LatestPatchedStreamingMaps, class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, name("CannonAnimationRedux" @ default.Class.name));
...
static function PatchAllLoadedMatinees()
{
	...
	foreach FoundMatinees(MatineeObject)
	{
		...
		`LOG(PackageNames[0] @ MatineeObject.ObjComment, class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, name("CannonAnimationRedux" @ default.Class.name));
...
static function PatchSingleMatinee(SeqAct_Interp SeqInterp)
{
	...
	foreach PatchAnimset.Sequences(Sequence)
	{
		...
		`LOG("Adding" @ name(Repl(Sequence.SequenceName, default.AnimSequencePrefix, "")), class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, name("CannonAnimationRedux" @ default.Class.name));
	}
	...
	foreach Data.InterpGroups(Group)
	{
		...
		//`LOG(Group.GroupName, class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, Class.name);
		if (UnitMapIndex != INDEX_NONE)
		{
			if(!class'X2DownloadableContentInfo_CannonAnimationRedux'.static.HasCannonEquipped(UnitMapping[UnitMapIndex].Unit))
			{
				`LOG(UnitMapping[UnitMapIndex].GroupName @ UnitMapping[UnitMapIndex].Unit.GetFullName() @ "!HasCannonEquipped, skipping patch", class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, name("CannonAnimationRedux" @ default.Class.name));
				continue;
			}
				...
				if (AnimControl != none)
				{
					for (KeyIndex = 0; KeyIndex <= AnimControl.AnimSeqs.Length; KeyIndex++)
					{
						if (PatchSequenceNames.Find(AnimControl.AnimSeqs[KeyIndex].AnimSeqName) != INDEX_NONE)
						{
							...
							`LOG("Patching" @ Group.GroupName @ UnitMapping[UnitMapIndex].Unit.GetFullName() @ AnimControl.AnimSeqs[KeyIndex].AnimSeqName, class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, name("CannonAnimationRedux" @ default.Class.name));
						}
						else
						{
			...
			if (UnitRef.ObjectID != 0) //Empty slots may be present in the squad as a 0 entry.
			{
				...
				if (GameStateUnit != none)
				{
					if(!GameStateUnit.bMissionProvided)
					{
						...
						UnitIndex++;
						//`LOG("Adding" @ GameStateUnit.GetFullName() @ "to unit mapping" @ NewMapping.GroupName, class'X2DownloadableContentInfo_CannonAnimationRedux'.default.bLog, Class.name);
					}
				}
			}
		}
...
defaultproperties
{
	...
	PatchAnimsetPath="Cannon_Cinematic.Anims.AS_Soldier"
}